...
// Aliyun config file path - only show claude-aliyun option if this exists
...
/**
 * Check if Aliyun config exists (private tool, only show if configured)
 */
function isAliyunConfigured() {
  try {
    if (!existsSync(ALIYUN_CONFIG_FILE)) return false;
...
export function getAvailableTools() {
  ...
  let builtins = BUILTIN_TOOLS.map(t => {
  ...
  // Conditionally add claude-aliyun if user has configured it
  if (isAliyunConfigured()) {
    builtins.push({
      id: 'claude-aliyun',
      name: 'Claude Code Aliyun',
      command: 'claude',
      builtin: true,
      available: isCommandAvailable('claude'),
    });
    console.log('[tools] claude-aliyun: available (configured)');
  }
